...
void CollisionSystem::AddGround(const std::vector<Eigen::Vector3d>& verts, const std::vector<int>& tris) {
  ground.BeginModel();
  AddToModel(ground, verts, tris);
  ground.EndModel();
}

void CollisionSystem::AddModel(int index,const std::vector<Eigen::Vector3d>& verts, const std::vector<int>& tris) {
  if (models.size() == index) {
    models.emplace_back();
  }
  if (models.size() > index) {
    models[index].BeginModel();
    AddToModel(models[index], verts, tris);
    models[index].EndModel();
  }
}

void CollisionSystem::GetCollisions(std::vector<unsigned int>& vertexToFace, std::vector<unsigned int>& edgeToEdge) {
  PQP_REAL translation[3][3];
  translation[0][0] = 0;
  translation[1][0] = 0;
  translation[2][0] = 0;
  translation[0][1] = 0;
  ...
  for (int i = 0; i < models.size(); ++i) {
    ...
    PQP_Collide(&cres, translation, rotation, &(ground), translation, rotation, &(models[i]));
    for (int j = 0; j < cres.NumPairs(); j++) {
      float v0[3], v1[3], v2[3];
      float u0[3], u1[3], u2[3];
      Tri* tri1 = ground.tris[cres->pairs[j]->id1];
      Tri* tri2 = models[i].tris[cres->pairs[j]->id2];
      v0[0] = tri1.p1[0];
      v0[1] = tri1.p1[1];
      v0[2] = tri1.p1[2];
